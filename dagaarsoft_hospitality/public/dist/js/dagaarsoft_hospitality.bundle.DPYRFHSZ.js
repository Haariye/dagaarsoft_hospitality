(() => {
  // ../dagaarsoft_hospitality/dagaarsoft_hospitality/public/js/property_session.js
  ...
  window.dh_apply_property = function(frm) {
    if (!frm.fields_dict || !frm.fields_dict.property)
      return;
  ...
  // ../dagaarsoft_hospitality/dagaarsoft_hospitality/public/js/posa_overrides/hotel_room_mixin.js
  (function() {
    "use strict";
    function isPosaPage() {
    ...
    frappe.call = function(opts) {
      if (opts && opts.method && opts.args) {
        var m = opts.method || "";
        var isInvoiceCall = m.indexOf("update_invoice") !== -1 || m.indexOf("submit_invoice") !== -1 || m.indexOf("update_sales_order") !== -1 || m.indexOf("submit_sales_order") !== -1;
        if (isInvoiceCall) {
    ...
    function makeCard(id, title, innerHTML) {
      ...
      card.innerHTML = '<div class="invoice-section-heading" style="padding:6px 12px 2px"><h3 class="invoice-section-heading__title" style="font-size:12px;font-weight:600;opacity:0.7;text-transform:uppercase;letter-spacing:0.5px">' + title + '</h3></div><div style="padding:4px 12px 8px">' + innerHTML + "</div>";
      return card;
    }
    var selectStyle = "width:100%;padding:7px 10px;border:1px solid var(--pos-border-color,#e2e8f0);border-radius:6px;font-size:13px;background:var(--pos-input-bg,#fff);color:var(--pos-text-primary,#2d3748);outline:none;transition:border-color 0.2s";
    var clearBtnStyle = "padding:4px 8px;background:#fed7d7;border:none;border-radius:4px;cursor:pointer;font-size:11px;color:#e53e3e;display:none";
    ...
    function injectHotelRoom(stores) {
      ...
      if (!anchor)
        return;
      var html = '<div style="display:flex;gap:6px;align-items:center"><div style="flex:1;min-width:0"><select id="dg-room-select" style="' + selectStyle + '"><option value="">-- Select Room --</option></select></div><div id="dg-guest-info" style="min-width:90px;font-size:11px;display:none"><div id="dg-guest-name" style="font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis"></div><div id="dg-guest-balance" style="color:#e53e3e;font-size:10px;display:none"></div></div><div id="dg-hotel-links" style="display:none;gap:4px;align-items:center;flex-shrink:0"><a id="dg-link-stay" href="#" target="_blank" style="' + linkStyle + ';border:1px solid rgb(var(--v-theme-primary,66,133,244));color:rgb(var(--v-theme-primary,66,133,244))">Stay</a><a id="dg-link-folio" href="#" target="_blank" style="' + linkStyle + ';border:1px solid #ed8936;color:#ed8936">Folio</a></div><button id="dg-clear-room" style="' + clearBtnStyle + '">\u2715</button></div>';
    ...
    function bindHotelRoom(card, stores) {
      ...
      select.addEventListener("change", function() {
        ...
        _originalFrappeCall({
          ...
          callback: function(r) {
            ...
            if (info.guest_stay) {
              linkStay.href = frappe.utils.get_url_to_form("Guest Stay", info.guest_stay);
              linkStay.style.display = "inline-block";
            } else {
              linkStay.style.display = "none";
            }
            if (info.guest_folio) {
              linkFolio.href = frappe.utils.get_url_to_form("Guest Folio", info.guest_folio);
      ...
      loadRooms();
      setInterval(loadRooms, 12e4);
    }
    function injectRestaurantTable(stores) {
      if (document.getElementById("dg-table-widget"))
        return;
      var anchor = document.getElementById("dg-hotel-room-widget") || findCustomerCard();
      if (!anchor)
        return;
      var html = '<div style="display:flex;gap:6px;align-items:center"><div style="flex:1;min-width:0"><select id="dg-table-select" style="' + selectStyle + '"><option value="">-- Select Table --</option></select></div><div id="dg-table-info" style="font-size:11px;display:none;white-space:nowrap"><span id="dg-table-status" style="font-weight:600"></span><span id="dg-table-cap" style="margin-left:6px;opacity:0.7"></span></div><button id="dg-clear-table" style="' + clearBtnStyle + '">\u2715</button></div>';
    ...
    function bindRestaurantTable(card, stores) {
      ...
      select.addEventListener("change", function() {
        ...
        var tbl = tablesCache.find(function(t) {
          return t.name === select.value;
        });
        if (tbl) {
          tableInfo.style.display = "block";
          var statusColor = tbl.status === "Available" ? "#276749" : tbl.status === "Occupied" ? "#e53e3e" : "#ed8936";
          ...
          if (tbl.floor)
            tableCap.textContent += (tableCap.textContent ? " \xB7 " : "") + tbl.floor;
          clearBtn.style.display = "inline-block";
        }
        frappe.show_alert({
      ...
      pollFor(function() {
        ...
        pollFor(function() {
          ...
          if (profile && profile.name)
            return { stores: s, profile };
          return null;
        }, 500, 120).then(function(result) {
          var stores = result.stores;
          var profile = result.profile;
          if (profile.posa_enable_hotel_room)
            injectHotelRoom(stores);
          if (profile.posa_enable_restaurant_table)
            injectRestaurantTable(stores);
...
//# sourceMappingURL=dagaarsoft_hospitality.bundle.DPYRFHSZ.js.map
