...
function _extend_stay(frm) {
    var current_dep = frm.doc.departure_date;
    var d = new frappe.ui.Dialog({
        title: __('Extend Stay'),
        fields: [
            {fieldtype:"HTML", options:
                "<p>Current departure: <b>" + (current_dep || "Not set") + "</b>"
                + " | Nights: <b>" + (frm.doc.num_nights || 0) + "</b></p>"
            },
            {fieldname:"new_departure_date", fieldtype:"Date",
             label:__("New Departure Date"), reqd:1,
             default: frappe.datetime.add_days(current_dep, 1)},
        ...
        primary_action: function(v) {
            frappe.call({
                ...
                args: {
                    stay_name: frm.doc.name,
                    new_departure_date: v.new_departure_date,
                    reason: v.reason || ""
                },
                freeze: true,
                callback: function(r) {
                    ...
                    if (r.message) {
                        frappe.show_alert({
                            message: __("Extended by {0} night(s). New departure: {1}",
                                [r.message.added_nights, r.message.new_departure]),
                            indicator: "green"
                        });
                    }
                    frm.reload_doc();
...
function _charge_to_credit(frm) {
    if (!frappe.user_roles.includes("Hotel Manager")) {
        frappe.msgprint(__("Only Hotel Manager can authorize charge-to-credit."));
        return;
    }
    var bal = flt(frm.doc.balance_due || 0);
    if (!frm.doc.guest_folio) {
        frappe.msgprint(__("No folio linked."));
        return;
    }
    // Fetch current folio balance
    frappe.db.get_value("Guest Folio", frm.doc.guest_folio, "balance_due", function(r) {
        var balance = parseFloat(r && r.balance_due || 0);
        if (balance <= 0) {
            frappe.msgprint(__("No outstanding balance to charge to credit."));
            return;
        }
        var d = new frappe.ui.Dialog({
            ...
            fields: [
                {fieldtype:"HTML", options:
                    "<div style='background:#fff3cd;padding:12px;border-radius:6px;margin-bottom:12px'>"
                    + "<p><b>Outstanding Balance: " + balance.toLocaleString("en",{minimumFractionDigits:2}) + "</b></p>"
                    + "<p>This will create a Journal Entry that:</p>"
                    + "<ul><li>Records the debt owed by <b>" + (frm.doc.billing_customer || frm.doc.customer) + "</b></li>"
                    + "<li>Keeps daily cash closing accurate</li>"
                    + "<li>Allows the guest to checkout</li></ul>"
                    + "</div>"
                },
                {fieldname:"reason", fieldtype:"Small Text",
                 label:__("Reason (required)"), reqd:1}
            ],
            primary_action_label: __("Authorize Charge to Credit"),
            primary_action: function(v) {
                frappe.call({
                    method: "dagaarsoft_hospitality.dagaarsoft_hospitality.doctype.guest_stay.guest_stay.charge_to_credit",
                    args: {stay_name: frm.doc.name, reason: v.reason},
                    freeze: true,
                    ...
                    callback: function(r) {
                        ...
                        if (r.message) {
                            frappe.show_alert({
                                message: __("JE {0} created. Guest can now checkout.", [r.message.journal_entry]),
                                indicator: "green"
                            });
                        }
                        frm.reload_doc();
