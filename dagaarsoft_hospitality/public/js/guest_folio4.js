...
frappe.ui.form.on("Guest Folio", {
    refresh(frm) {
        ...
        // Account Statement always visible on submitted folios (even after checkout/closed)
        if (frm.doc.docstatus === 1 && frm.doc.folio_status !== "Open") {
...
function _add_statement_btn(frm) {
    // Standalone Account Statement button for closed/checked-out folios
    frm.add_custom_button(__("Account Statement"), function() {
        frappe.call({
            method: "dagaarsoft_hospitality.dagaarsoft_hospitality.doctype.guest_folio.guest_folio.get_folio_summary",
            args: {folio_name: frm.doc.name},
            callback: function(r) {
                ...
                if (ledger.length) {
                    ...
                    ledgerTableHtml = "<p style='color:#a0aec0;font-size:12px'>No financial transactions.</p>";
                }
                var trueBalance = flt(s.true_balance || 0);
                var balColor = trueBalance > 0.01 ? "#c53030" : "#276749";
                frappe.msgprint({title:__("Guest Account Statement (Archived)"), wide:true, message:
                    "<div style='font-family:Arial,sans-serif;font-size:13px'>"
                    + "<div style='background:#e2e8f0;padding:8px 12px;border-radius:4px;margin-bottom:12px;font-size:11px;color:#4a5568'>This folio is <b>" + (frm.doc.folio_status||"Closed") + "</b>. Read-only view.</div>"
                    + "<div style='font-size:16px;font-weight:bold;margin-bottom:4px'>" + (s.guest||"") + "</div>"
                    + "<div style='color:#718096;margin-bottom:12px'>Room " + (s.room||"") + " | " + (s.arrival||"") + " → " + (s.departure||"") + " (" + (s.nights||0) + " nights)</div>"
                    + "<div style='margin-bottom:16px'>"
                    + "<div style='font-size:11px;font-weight:700;color:#4a5568;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:6px;border-bottom:2px solid #e2e8f0;padding-bottom:4px'>Charges Breakdown</div>"
                    ...
                    + "<div style='margin-bottom:12px'>"
                    + "<div style='font-size:11px;font-weight:700;color:#4a5568;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:6px;border-bottom:2px solid #e2e8f0;padding-bottom:4px'>Financial Transactions</div>"
                    + ledgerTableHtml + "</div>"
                    + "<div style='background:#f7fafc;border:1px solid #e2e8f0;border-radius:6px;padding:12px 16px;display:flex;justify-content:space-between;align-items:center'>"
                    + "<div style='font-weight:700'>Final Balance</div>"
                    + "<div style='font-size:20px;font-weight:bold;color:" + balColor + "'>" + fc(Math.abs(trueBalance)) + (trueBalance < -0.01 ? " CR" : "") + "</div>"
                    + "</div></div>"
                });
            }
        });
