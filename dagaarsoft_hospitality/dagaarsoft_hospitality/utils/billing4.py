...
def validate_checkout_billing(guest_stay_name, force_checkout=False):
    ...
    Strict checkout validation:
      1. Total charges must equal total invoiced amount
      2. Total invoiced must equal total paid (payments)
      3. Block checkout unless all three match — EXCEPT:
         - Sponsored stays (billing_customer != guest) → always allowed with warnings
         - Hotel Manager "charge_to_credit" was used → JE covers the gap
         - force_checkout=True with Hotel Manager role → early checkout with void
    """
    stay = frappe.get_doc("Guest Stay", guest_stay_name)
    ...
    if is_early_checkout:
        remaining_nights = date_diff(str(stay.departure_date), today())

    # ── Calculate the three pillars ───────────────────────────────────────
    # 1. Total charges (non-void)
    total_charges = sum(flt(c.amount) for c in (folio.get("folio_charges") or [])
                        if not c.is_void)

    # 2. Total invoiced (submitted Sales Invoices linked to this folio)
    ...
    # 3. Total paid (submitted Payment Entries linked to this folio)
    ...
    # Also count Journal Entries (charge-to-credit)
    total_je_credit = flt(frappe.db.sql("""
    ...
    effective_paid = total_paid + total_je_credit
    ...
    if is_sponsored:
        # Sponsored checkout: always allowed, just warn
        ...
        # ── Strict validation: charges = invoices = payments ──────────────
        force = bool(int(force_checkout))

        # Check 1: All charges must be invoiced
        if unbilled and not force:
            issues.append(_("{0} charge(s) totalling {1} not yet invoiced. Generate invoice first.").format(
                len(unbilled),
                frappe.format_value(unbilled_total, {"fieldtype": "Currency"})))

        # Check 2: Invoiced total must match charges
        if abs(net_invoiced - total_charges) > 0.01 and not unbilled and not force:
            issues.append(_("Charges ({0}) ≠ Invoiced ({1}). Reconcile before checkout.").format(
                frappe.format_value(total_charges, {"fieldtype": "Currency"}),
                frappe.format_value(net_invoiced, {"fieldtype": "Currency"})))

        # Check 3: All invoices must be fully paid
        if total_outstanding > 0.01 and not force:
            issues.append(_("Unpaid invoice balance: {0}. Settle payment or use 'Charge to Credit'.").format(
                frappe.format_value(total_outstanding, {"fieldtype": "Currency"})))
