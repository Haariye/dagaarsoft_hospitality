...
class RoomMove(Document):
    ...
    def _auto_fill(self):
        ...
        self.old_nightly_rate = flt(stay.nightly_rate)
    ...
    def on_submit(self):
        ...
        if stay.guest_folio:
            ...
            if diff != 0 and rem > 0:
                # FIX 6: Check if this room_move billing already posted to folio
                ...
                if not _charge_exists(stay.guest_folio, "Room Move", self.name):
                    ...
                    if folio.folio_status == "Open":
                        ...
                        desc = "{0}: {1}->{2} | {3}{4}/n x {5}n = {6}".format(
                            ...
                            sign, round(diff, 2), int(rem), round(diff * rem, 2))
                        if diff < 0:
                            desc += " [Refund via JV at checkout]"
                        line = folio.append("folio_charges", {})
                        line.description = desc; line.qty = flt(rem)
                        line.rate = flt(diff); line.amount = flt(diff * rem)
                        line.charge_category = cat
                        line.posting_date = str(self.move_date or today())
                        line.posting_time = nowtime()
                        line.reference_doctype = "Room Move"; line.reference_name = self.name
                        line.guest_stay = self.guest_stay; line.posted_by = frappe.session.user
                        line.is_read_only = 1
